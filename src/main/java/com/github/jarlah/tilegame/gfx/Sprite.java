package com.github.jarlah.tilegame.gfx;
...
public class Sprite {
	...
	public BufferedImage loadSprite(String link) {
		...
		try {
			return sprite = ImageIO.read(Sprite.class.getClassLoader()
	...
	public BufferedImage loadSprite(String name, int x, int y) {
		...
		if (imageCache.containsKey(name) && imageCache.get(name).x == x
				&& imageCache.get(name).y == y) {
			return imageCache.get(name).img;
		}

		BufferedImage img = sprite.getSubimage(x * TILE_SIZE, y * TILE_SIZE,
				TILE_SIZE, TILE_SIZE);
